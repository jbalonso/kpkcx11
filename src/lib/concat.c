...
/* first arg is either a separator that goes between args (including
   empty strings) or a -1 to specify no separator; then we just do
   plain concatenation. */
char *concat(int sep,...)
{
  va_list sp;
  ...
  *string='\0';
  va_start(sp,sep);
  while ((ptr=va_arg(sp,char*)) !=NULL) {
    slen=strlen(string);
    plen=strlen(ptr);
    if (plen+slen+2>len) { /* sep before, null after */
      if ((string=realloc(string,plen+slen+256))==NULL) {
      }
      len=plen+slen+256;
    }
    if (sep!=-1) {
