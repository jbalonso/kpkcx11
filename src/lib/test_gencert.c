...
X509 *gencert(NETSCAPE_SPKI *spkac, struct a_t **tattrl, char *keyfile, 
  ...
  if (!user) user=strdup("anonymous"); /* uh huh. */
...
int add_nentry(X509_NAME *name,char *string,int nid,int i)
{
  ...
  nentry->value->data=(unsigned char *)strdup(string);
