...
int main()
{
	CK_ULONG hSession=0L;
	int	rc;
	CK_FUNCTION_LIST_PTR pFunctionList;
	
	rc = C_GetFunctionList(&pFunctionList);
	printf("C_GetFunctionList returned %0d\n", rc);

	if ( (rc == 0) && (pFunctionList->C_Initialize) ) {
		/* resolve call through function list pointer */
		rc = (pFunctionList->C_Initialize)(NULL_PTR);
	} else {
		/* let MacOS resolve call  */
		rc = C_Initialize(NULL_PTR);
	}
	printf("C_Initialize returned %0d\n", rc);

	rc = C_OpenSession(1, CKF_SERIAL_SESSION, NULL_PTR, NULL_PTR, &hSession);
	printf("C_OpenSession returned %0d\n", rc);
